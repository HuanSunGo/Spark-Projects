...
predDF_final.write.format('jdbc').options(
      url='jdbc:mysql://ne-gr5069.ccqalx6jsr2n.us-east-1.rds.amazonaws.com/gr_5069',
      driver='com.mysql.jdbc.Driver',
      dbtable='hs_gr5069_airbnb_prediction',
      user='admin',
      password='columbia').mode('overwrite').save()

# COMMAND ----------

predDF_final_done = spark.read.format("jdbc").option("url", "jdbc:mysql://gr5069-ne.ccqalx6jsr2n.us-east-1.rds.amazonaws.com/gr_5069") \
    .option("driver", "com.mysql.jdbc.Driver").option("dbtable", "test_airbnb_preds_bath_bed") \
    .option("user", "admin").option("password", "columbia").load()
...
predDF_final_bed = spark.read.format("jdbc").option("url", "jdbc:mysql://gr5069-ne.ccqalx6jsr2n.us-east-1.rds.amazonaws.com/gr_5069") \
    .option("driver", "com.mysql.jdbc.Driver").option("dbtable", "test_airbnb_preds") \
    .option("user", "admin").option("password", "columbia").load()

# COMMAND ----------

display(predDF_final_bed)

# COMMAND ----------

import mlflow
logged_model = 'runs:/b31279363ca34d0fb2c31b7a7e190222/linear-regression-model'

# Load model as a Spark UDF.
loaded_model = mlflow.pyfunc.spark_udf(spark, model_uri=logged_model)
columns = list(predDF_final.columns)

# Predict on a Spark DataFrame.
predDF_final.withColumn('new_predictions', loaded_model(*columns)).collect()
