...
# the wrapper function can help us to further iterate, and log later 
with mlflow.start_run(run_name="Basic RF Experiment") as run:
  ...
  # give the unique things about this model, history about what have been done about ML flow
...
# MAGIC %md
# MAGIC can see the details of the model by clicking the bottle button on the top right of the screen
...
def log_rf(experimentID, run_name, params, X_train, X_test, y_train, y_test):
  ...
  with mlflow.start_run(experiment_id=experimentID, run_name=run_name) as run:
    # Create model, train it, and create predictions
    rf = RandomForestRegressor(**params) # this is where further send parms into
    rf.fit(X_train, y_train)
    predictions = rf.predict(X_test)
...
# now just call the function, and change the parameter by assigning different params metrics
...
# MAGIC 
# MAGIC - For future thing: can do the grid search also by loop-log-call cycle.
